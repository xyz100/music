...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refresh(){//singer.vue里调用
        this.$refs.listview.refresh()
      },
      scroll(pos) {
        this.scrollY = pos.y
      },
      _scrollTo(index) {//跳转到对应位置
        if(!index && index!==0){
          return
        }
        if(index < 0){
      ...
      _calculateHeight() {//计算每个区块高度
        ...
        for(let i = 0; i < list.length; i++){
          let item = list[i]
          height += item.clientHeight //每个li的高度
          this.listHeight.push(height)
        }
      }
    },
